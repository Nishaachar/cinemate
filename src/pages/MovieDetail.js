...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie () {
      try{
        const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=7032820800597be2453ee2d3b84e8f21
        `);
    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        ...
        <p className="my-4">
          ...
          <a className="hover:underline" href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank" rel="noreferrer">{movie.imdb_id}</a>
        </p>
